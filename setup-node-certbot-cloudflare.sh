...
detect_identity() {
  ...
  tty_read -r -p "Node number (for example 5, 6, 13): " NODE_NUM
...
get_cloudflare_credentials() {
  ...
  while :; do
    tty_read -r -s -p "Cloudflare API Token (DNS Edit for $ZONE): " token
    printf '\n'
    [[ -n $token ]] || { warn "Token cannot be empty."; continue; }
    tty_read -r -s -p "Repeat API Token: " token2
    printf '\n'
...
create_combined_certificate() {
  ...
  if ! has_certbot_account; then
    tty_read -r -p "Email for Let's Encrypt account: " email
